...
the challenge, lives in [`code/`](../code/) — `Crop.cs`, `Animal.cs`, and
`Program.cs` — in case you get stuck or want to compare once you're done.
...
thing, is **encapsulation** — the same idea Course 2 already introduced,
now with an actual rule behind it instead of just a bundle of fields. More:
